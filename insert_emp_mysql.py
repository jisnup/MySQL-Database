...
df = pd.read_excel('C:/Users/jisnu/Desktop/jisnu/data/Employee Details.xlsx')

df.columns = [c.lower() for c in df.columns]
df.columns = [c.replace(" ", "_") for c in df.columns]
df.drop(['state','220','215'], axis = 1, inplace  = True)
...
df = df[['license', 'first_name','last_name', 'hire_date','birthday', 'gender', 'position','department', 'office_location' ,'payroll_department' ,'source' ,'sub_code' ,'life_writing',
        'shirt_size' ,'recruited' ,'previously_licensed']]
...
"""
    ...
    license, first_name,last_name, hire_date,birthday, gender, position,department, office_location ,payroll_department ,source ,sub_code ,life_writing ,shirt_size ,recruited ,previously_licensed) 
    SELECT DISTINCT license, first_name,last_name, hire_date,birthday, gender, position,department, office_location ,payroll_department ,source ,sub_code ,life_writing ,shirt_size ,recruited ,previously_licensed FROM employee_temp 
