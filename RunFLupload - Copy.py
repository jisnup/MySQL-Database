...
engine = create_engine('mysql+mysqlconnector://XXXXXXXXX@XXXXXXXXX:XXXX/florida', echo = False)
conn = engine.connect()
...
try:
	...
	df.loc[df.sub_producer == 'No Code', 'sub_producer'] = '000'
...
try:
	...
	conn.execute(""" TRUNCATE TABLE fl_sales_temp; """)
	# Moving the file to archive after insertion with the date for the tracking
...
try:
	...
	df.columns = [c.replace(" ", "_") for c in df.columns]
	# Dropping extra data
	df.drop(['unnamed:_0','unnamed:_20','street_address','city','state','zip_code','email', 'insured_first_name','insured_last_name','phone_number',
...
try:
	...
	print('\n 9."Call History" insertion compelete after: {} seconds and moved to Archive!!'.format(time.time() - start_time))
except:
